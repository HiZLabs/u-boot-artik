...
/* Samsung OTA not compatible with Mender */
#if defined(CONFIG_ARTIK_OTA)
#undef CONFIG_ARTIK_OTA
#endif
...
#define CONFIG_ROOT_PART_SD	1

#define CONFIG_ROOTA_PART	1
#define CONFIG_ROOTB_PART	2
#define CONFIG_ROOT_PART	CONFIG_ROOTA_PART
...
#define CONFIG_DFU_ALT \
	...
	"/boot/uImage ext4 $rootdev $rootpart;" \
	"/boot/Image ext4 $rootdev $rootpart;" \
	"/boot/uInitrd ext4 $rootdev $rootpart;" \
	"/boot/ramdisk.gz ext4 $rootdev $rootpart;" \
	"/boot/s5p6818-artik710-raptor-rev03.dtb ext4 $rootdev $rootpart;" \
	"/boot/s5p6818-artik710-raptor-rev02.dtb ext4 $rootdev $rootpart;" \
	"/boot/s5p6818-artik710-raptor-rev01.dtb ext4 $rootdev $rootpart;" \
	"/boot/s5p6818-artik710-explorer.dtb ext4 $rootdev $rootpart;" \
	"rootfs part $rootdev $rootpart;" \
	"rootfsa part " __stringify(CONFIG_ROOT_DEV) " " __stringify(CONFIG_ROOTA_PART) ";" \
	"rootfsb part " __stringify(CONFIG_ROOT_DEV) " " __stringify(CONFIG_ROOTB_PART) ";" \
	"params.bin raw 0x1701 0x20;" \
	"/boot/Image.itb ext4 $rootdev $rootpart\0"

#define CONFIG_EXTRA_ENV_SETTINGS					\
	"fdt_high=0xffffffffffffffff\0"					\
	"kerneladdr=0x40080000\0"					\
	"kernel_file=boot/Image\0"						\
	...
	"load_fdt="							\
		...
		"until test $loop -eq 0 || test $success -ne 0; do "	\
			...
			"else number=$loop; "				\
			"fi; "						\
			"ext4size ${mender_uboot_root} boot/s5p6818-artik710-raptor-rev${number}.dtb && setexpr success 1; " \
			"setexpr loop $loop - 1; "			\
		"done; "						\
		"if test $success -eq 0; then "				\
			"ext4load ${mender_uboot_root} $fdtaddr boot/s5p6818-artik710-raptor-rev00.dtb || "	\
			"ext4load ${mender_uboot_root} $fdtaddr boot/s5p6818-artik710-raptor.dtb; "	\
		"else "							\
			"ext4load ${mender_uboot_root} $fdtaddr boot/s5p6818-artik710-raptor-rev${number}.dtb; "	\
		"fi; "							\
		"else ext4load {mender_uboot_root} $fdtaddr $fdtfile; " \
	...
	"lcd_panel=s6e8fa0\0"						\
	"sdrecovery="				\
		"sd_recovery mmc 1:1 48000000 partmap.txt\0"	\
	...
	"load_args=run factory_load; setenv bootargs ${console} "	\
		"root=${mender_kernel_root} ${root_rw} "	\
		"rootfstype=${rootfs_type} ${opts} ${recoverymode} "	\
		"drm_panel=$lcd_panel "					\
		"rescue=${rescue};\0"	\
	"load_kernel="							\
		"ext4load ${mender_uboot_root} $kerneladdr $kernel_file;\0" \
	"boot_cmd_mmcboot="						\
		"run load_kernel; run load_fdt;"			\
		"booti $kerneladdr - $fdtaddr\0"			\
	"mmcboot=run mender_setup; run load_args; run boot_cmd_mmcboot; run mender_try_to_recover\0"			\
	"recovery_cmd=run sdrecovery;\0"	\
	"recoveryboot=run recovery_cmd; run mmcboot\0"		\
